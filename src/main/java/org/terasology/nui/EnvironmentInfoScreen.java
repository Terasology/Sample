    ...
    public void initialise() {
        ...
        if (updateInfoButton != null) {
            updateInfoButton.subscribe(button -> {
                ...
                infoArea.setText(String.format("Welcome to the environment info screen!%n" + "the current world has been active for %.0f (in-game) seconds.%n" + "Currently running at %.2f FPS and using %.2f MB of memory out of %.2f available.", time.getGameTime(), time.getFps(), memoryUsage, Runtime.getRuntime().maxMemory() / bytesInMegabyte));
            });
        }
    }
}